<?php
    ...
    $eventname_err=$description_err=$contact_err=$date_err=$questions_err=$attachment_err="";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty(trim($_POST["questions"]))){
            ...
            $num_of_questions = count($questions);
            $questions_data=implode(",",$questions);
         }
        ...
        function GetImageExtension($imagetype){
         ...
         if(move_uploaded_file($tmp_name,$target_path)){
          ...
          return $randomString;
                 ...
                 $form_name= generateRandomString();
               $query="INSERT INTO 
               events(event_name, description,contact_numbers,date_of_event,questions,event_poster, form_name, user_id, venue, council, time, registrations, delete_e)
               VALUES ('$event_name', '$description' , '$contact_numbers' , '$date_of_event' , '$questions_data' ,'$imagename' ,'$form_name' ,'$user_id','$venue', '$council', '$timing', 'Yes', 'No')";
                       ...
                       if(mysqli_query($link,$query)){
                           $x = 0;
                             ...
                             if(mysqli_query($link, $query)){
                                ...
                                $_SESSION['message'] = "Event Not Added. PLease try again.";
                            ...
                            <div class="content">
                                ...
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="form" enctype="multipart/form-data">
                                    ...
                                    <div class="form_item"  <?php echo (!empty($venue_err)) ? 'has-error' : ''; ?>>
                                        <label class="form_input">
                                            <input placeholder="Venue" name="venue" autocomplete="off" type="text" class="form_input_field"/>
                                            <p class="form_input_field--error"><?php echo $venue_err; ?></p>
                                        </label>
                                    </div>
                                    <div class="form_item"  <?php echo (!empty($time_err)) ? 'has-error' : ''; ?>>
                                        <label class="form_input">
                                            <input placeholder="Time" name="timing" autocomplete="off" type="text" class="form_input_field"/>
